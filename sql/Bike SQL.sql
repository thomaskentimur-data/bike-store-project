...
alter table staffs
add column nama_staff varchar(255);

update staffs
set nama_staff = concat(`first_name`, ' ', `last_name`);

alter table staffs
modify column nama_staff varchar(255)
after staff_id;
...
alter table staffs
change nama_staff staff_name varchar(255);
...
select *,
row_number() over(partition by nama,order_date, required_date, shipped_date, store_name, staff_name, product_name, model_year, category_name, brand_name,quantity, list_price, discount, total) as row_num
from trans;
